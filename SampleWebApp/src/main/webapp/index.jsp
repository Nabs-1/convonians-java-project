<!DOCTYPE html>
<html>
  <head>
    <title>Welcome to my New Java Project this Year 2023</title>
    <style>
      body { background-color: #F4C2C2; } /* Baby Pink */
    </style>
  </head>
  <body>
    <h1>Nabilah Tomcat Project</h1>
    <!--Form declaration. Must have an action attribute -->
    <form action="/submit" method="post">
      <p>Let us learn something about your music lifestyle</p>
      <textarea name="comments" cols="30" rows="5"></textarea>

      <p>Select your favourite genre of music<br />
        <input type="radio" name="genre" value="rock" checked="checked"/>Rock <br />
        <input type="radio" name="genre" value="pop"/>Pop <br />
        <input type="radio" name="genre" value="reggae"/>Reggae <br />
        <input type="radio" name="genre" value="jazz"/>Jazz <br />
      </p>

      <p>Select your favourite music service<br />
        <input type="checkbox" name="service[]" value="iTunes"/>iTunes <br />
        <input type="checkbox" name="service[]" value="Spotify"/>Spotify <br />
        <input type="checkbox" name="service[]" value="Pandora"/>Pandora <br />
        <input type="checkbox" name="service[]" value="Fish FM"/>Fish FM <br />
      </p>
      
      <p>What device do you listen to music on<br />
        <select name="devices">
          <option value="ipod">iPod</option>
          <option value="radio">Radio</option>                     
          <option value="tv">TV</option>
          <option value="phone">Phone</option>
          <option value="computer">Computer</option>
          <option value="echo">Echo</option>
        </select>
      </p>
      
      <p>Select the instruments you're capable of playing. Hold CTRL key to select multiple</p>
      <select name="instruments" multiple>
        <option value="guitar">Guitar</option>
        <option value="drum">Drum</option>
        <option value="keyboard">Keyboard</option>
        <option value="shekere">Shekere</option>
        <option value="trumpet">Trumpet</option>
      </select>  
      
      <p>Upload your Song in MP3 format only.</p>
      <input type="file" name="user_song" accept=".mp3" /><br />
      <input type="submit" value="Upload" />
      
      <p>Enter Your Email to subscribe to our weekly newsletter.</p>
      <input type="email" name="email" />
      <input type="button" name="subscribe" value="Subscribe" />
      
      <br /> <br />
      <label>Age: <input type="text" name="age" /></label> <br />
      Gender:
      <input id="female" type="radio" name="gender" value="f"/>
      <label for="female">Female</label>
      <input id="male" type="radio" name="gender" value="m"/>
      <label for="male">Male</label>
      <input id="other" type="radio" name="gender" value="o"/>
      <label for="other">Other</label>
    </form> 
    
    <fieldset>
      <legend><strong>Bio Data</strong></legend>
      <label>First Name:
        <input type="text" name="fname" />
      </label><br /><br />
      <label>Last Name:
        <input type="text" name="lname" />
      </label><br /><br />
      <label>D.O.B:
        <input type="date" name="dob" />
      </label><br /><br />
      <fieldset>
        <legend><strong>Contact Info</strong></legend>
        <label>Mobile No:
          <input type="text" name="mobile" />
        </label>
        <label>Email:
          <input type="email" name="email" />
        </label><br /><br />
        <label>Address:
          <input type="text" name="address" />
        </label>
        <label>City:
          <input type="text" name="city" />
        </label><br /><br />
        <label>State:
          <select name="state">
            <option value="Alabama">Alabama AL</option>
            <option value="Texas">Texas TX</option>                     
            <option value="Tennessee">Tennessee TN</option>
            <option value="Arkansas">Arkansas AR</option>
            <option value="Arizona">Arizona AZ</option>
            <option value="Louisiana">Louisiana LA</option>
          </select>
        </label>
        <label>Zip Code:
          <input type="text" name="zipcode" />
        </label>
      </fieldset>
    </fieldset>
    
    <p><strong>Search Here</strong></p>
    <input type="search" name="search" placeholder="Enter search keyword" />
    <button type="submit">Search</button>
  </body>
</html>
